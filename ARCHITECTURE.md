...
| 3 | ✅ Google 로그인 + 계정 연결 UI (코드 완료 — 콘솔 설정 필요: 익명·Google 프로바이더, 승인 도메인, Firestore+규칙) | **★ 목표 달성: 다른 기기/IP에서 이어 하기** |
...
### 9.8 닉네임 — 공개 식별자 (추후 랭킹용)

> 목표: 계정마다 표시 이름을 갖는다. 2~9자, 전역 유일, 자유롭게 변경 가능. 세이브(§6)와 분리된 **공개** 데이터 — 추후 서버 리더보드(§8)가 uid→이름을 여기서 읽는다.
